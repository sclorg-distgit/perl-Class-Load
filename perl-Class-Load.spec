%{?scl:%scl_package perl-Class-Load}
%{!?scl:%global pkg_name %{name}}

Name:		%{?scl_prefix}perl-Class-Load
Version:	0.20
Release:	8%{?dist}
...
BuildRequires:	%{?scl_prefix}perl(Class::Load::XS)
%endif
...
BuildRequires:	%{?scl_prefix}perl(Pod::Coverage::Moose)
%endif
...
%check
%if 0%{!?perl_bootstrap:1} && 0%{?rhel} < 7 && ! 0%{?scl:1}
%{?scl:scl enable %{scl} "}
...
%changelog
* Mon Nov 25 2013 Jitka Plesnikova <jplesnik@redhat.com> - 0.20-8
- Re-rebuild of bootstrapped packages

* Sun Nov 24 2013 Jitka Plesnikova <jplesnik@redhat.com> - 0.20-7
- Rebuilt for SCL
